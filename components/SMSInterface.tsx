...
import { Send, Image as ImageIcon, Smile, Search, MoreVertical, Phone, Plus, X, Settings, Edit2, Trash2, ChevronDown, RefreshCw, Inbox } from 'lucide-react';
...
interface ComposeBarProps {
   ...
   * so a refused send leaves the user's text intact — a send that did not
   * happen must never look like it worked. (The free-tier daily cap was the
   * original refuser; it is gone as of trial-caps-purge 2026-09-21. The
   * contract stays: "no recipient" still refuses, and the callers in
   * PhoneModeShell rely on the boolean.)
   */
...
const ComposeBar = memo(function ComposeBar({ onSend, disabled }: ComposeBarProps) {
  ...
  const send = useCallback(() => {
    ...
    if (!accepted) return; // refused → keep the text; do NOT clear
    setMessageText('');
...
export const SMSInterface = ({ initialNumber }: SMSInterfaceProps = {}) => {
  ...
  // Returns whether the send was admitted; the compose box (ComposeBar) only
  // clears on true, so a refused send keeps the user's text.
