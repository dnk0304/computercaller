...
try {
  ...
  if (KEEPALIVE_KNOB_OFF) {
    console.log('  [control arm] cc-keepalive LEFT ARMED + token removed for the whole indicator block');
    /**
     * THE CONTROL ARM: THE TOKEN IS ACTUALLY GONE, NOT MOCKED AWAY.
     *
     * The arm leaves cc-keepalive ARMED (11z asserts it) and makes the worker's
     * token reads come back null for the whole block — which is the state the
     * keepalive path used to paint 'signed-out' over, and the exact code path
     * this lane changed. The worker's own "I have held a token" fact is
     * established FIRST, as a real sign-in would, so what follows is a read
     * racing a write rather than a signed-out profile.
     *
     * TWO EARLIER SHAPES OF THIS ARM WERE WRONG; BOTH COST A RUN.
     *
     * 1. Wrapping chrome.storage.local.get to hide the key. It is callback AND
     *    promise flavoured and is called from more places than this block knows
     *    about; the wrapper hung the worker outright with no error anywhere.
     *    REMOVING the key needs no wrapper: getToken() returns null through the
     *    shipped code path, which is what is under test anyway.
     *
     * 2. Letting the read succeed after ~1 s. `e2e-harness-token` is not a real
     *    JWT, so a connect() that gets past the read either mints a ticket and
     *    is refused 401 — an AUTHORITATIVE revocation the fix deliberately
     *    honours — or fails the fetch and correctly calls clearRelayFacts(),
     *    which wipes the phonePresent this block is asserting. Both are the
     *    product behaving properly; both make the arm measure something other
     *    than the null-token repaint. Observed as 11a reading
     *    'signed-in-disconnected' instead of 'phone-unpaired'.
     *
     * So: establish the fact, then take the token away and leave it away. Every
     * keepalive tick then stops exactly where the bug lived, at
     * `const token = await getToken()` returning null.
     */
    await sw.evaluate(async () => {
      await refreshAuthAndIndicator();
      if (!signedIn) throw new Error('control arm: seeded token was not picked up before removing it');
  ...
  const armIndicator = () => sw.evaluate(async (knobOff) => {
    ...
    lastIndicator = null;
    refreshIndicator();
    if (knobOff && lastIndicator === 'signed-out') {
      // The tell from the ISSUES entry, asserted at the arm rather than left to
      // surface as a puzzling assertion failure three checks later.
