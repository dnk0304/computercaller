 ...
 */
export const CC_FT_DB_VERSION = 2;
...
/**
 * v2 (FILE-QUEUE-WEB). The outgoing transfer queue's METADATA — id, name, size,
 * lastModified, direction, state, reason — as one snapshot record. Never a
 * File and never bytes: after a restart a queued row comes back as needs-file.
 * Additive: the v1 -> v2 upgrade creates this store and leaves `resume` alone.
 */
export const CC_FT_STORE_QUEUE = 'queue';

/** Every store in the cc-ft v2 schema. The upgrade body is driven off this list. */
export const CC_FT_STORES = Object.freeze([CC_FT_STORE_RESUME, CC_FT_STORE_QUEUE]);
...
// ── THE THIRD DATABASE: `cc-read` ───────────────────────────────────────────
//
// Per-thread "I opened this conversation ON THIS COMPUTER" state. Same reasoning
// as `cc-ft` above, applied to a different lifecycle — so the same shape, not a
// new mechanism:
//
//   LIFECYCLE.  An opened-marker is a UI convenience. It is rewritten every time
//               a thread is opened and it is deliberately DESTROYED on sign-out
//               and on "Forget this computer". `deviceKey` is the opposite: it
//               must survive every one of those except the two explicit revokes.
//               A store that churns on every click does not belong in the same
//               upgrade scope as the key that holds the pairing.
//
//   VERSIONING. Adding a read-state store to `cc-e2e` bumps CC_E2E_DB_VERSION,
//               and a rollback to a build without it then hits the deliberate
//               VersionError and cannot open the device key AT ALL. Unread
//               ticks must never be able to un-pair a browser.
//
//   BLAST RADIUS. `cc-read` blocked or corrupt degrades to "nothing is marked
//               unread this session" — the list still renders, pairing is
//               untouched. The same fault in `cc-e2e` aborts pairing.
//
//   DISPOSAL.   Sign-out deletes this account's keys by prefix. On a shared
//               database that same gesture would reach the device key.
//
// The phone does not send read state (SmsMessage has no `read` field and the
// SMS_RECEIVED / MESSAGES_CHUNK frames carry none), so there is nothing to
// mirror and nothing to sync back. This database IS the claim, and it is a
// claim about this browser profile only.
//
// Open logic, version guard, additive upgrade and transaction discipline are
// `openDatabase` above, shared verbatim.

/** The one thread-read-state database name. Do not write this string anywhere else. */
export const CC_READ_DB_NAME = 'cc-read';

/**
 * The read-state schema version. Independent of CC_E2E_DB_VERSION and
 * CC_FT_DB_VERSION by design — see the block above. Bump this, and add the
 * store to {@link CC_READ_STORES} in the same edit; the upgrade body needs no
 * change.
 */
export const CC_READ_DB_VERSION = 1;

/**
 * Per-thread opened markers, keyed OUT-OF-LINE by the string
 * `<userId>|<threadKey>` — plus one `<userId>|__baseline` record per account.
 * Same convention as every `cc-e2e` store, so `openDatabase`'s one
 * `createObjectStore(name)` upgrade body covers all three databases.
 */
export const CC_READ_STORE_THREAD_OPENED = 'threadOpened';

/** Every store in the cc-read v1 schema. The upgrade body is driven off this list. */
export const CC_READ_STORES = Object.freeze([CC_READ_STORE_THREAD_OPENED]);

const CC_READ_SCHEMA = defineDatabase(CC_READ_DB_NAME, CC_READ_DB_VERSION, CC_READ_STORES);

/** Open `cc-read` at the current schema version. See {@link openDatabase}. */
export function openCcReadDb(factory) {
  return openDatabase(CC_READ_SCHEMA, factory);
}

/** Read one store of `cc-read`. See {@link idbRead}. */
export function ccReadRead(factory, storeName, fn) {
  return idbRead(CC_READ_SCHEMA, factory, storeName, fn);
}

/** Write one store of `cc-read`. See {@link idbWrite}. */
export function ccReadWrite(factory, storeName, fn) {
  return idbWrite(CC_READ_SCHEMA, factory, storeName, fn);
}
