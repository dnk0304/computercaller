...
/**
 * Does this kid still have seq history in the store? (P2.6, Security MUST #1.)
 *
 * The A3-M2 floor is in the deviceKey store; the counters are in the seq store.
 * They can be lost INDEPENDENTLY — a partial clear, a restore of one object
 * store, a profile copy. When they diverge, an equal-epoch RESUME is admitted
 * against a floor that survived while the counter that bounds it did not, and
 * the session would then be built with `fresh:false` on nothing, or worse with
 * `fresh:true` and a counter back at 0 under a kid the phone believes has
 * already sealed. `bindKid` refuses the second half of that (no record +
 * resumed -> counter-lost), but the refusal belongs at the ADMISSION too:
 * whether the pair lives must not depend on a `fresh` flag computed correctly
 * three calls later.
 *
 * Either direction counts. One surviving counter is proof the kid has history;
 * the send direction is the one that can burn a nonce, but a p2c record alone
 * is still evidence this is not a virgin store.
 */
export async function hasSeqRecord({ store, kid }) {
  if (typeof kid !== 'string' || kid.length === 0) return false;
  for (const direction of [DIR_C2P, DIR_P2C]) {
    const rec = await store.load(seqId(kid, direction));
...
export function createFailClosedSender({ store, kid, direction, floor, sk }) {
  ...
  let poisoned = null;
  /**
   * T-FT-WEB-CHUNK-SEQ-RACE. The commits are serialised through this tail so
   * they land in the order the sequences were reserved. Without it two
   * concurrent commits can settle out of order and the persisted floor would
   * describe a seq that is NOT the highest one handed out — the same
   * counter-reuse hazard, one reload later.
   */
  ...
  return {
    ...
    async nextSeq() {
      ...
      // This used to read `next`, await the commit, and only then advance.
      // `nextSeq()` is not called one-at-a-time: lib/fileTransfer/sender.ts
      // pumps N chunks into sealOutbound back-to-back and every one of them is
      // inside this function before the first commit resolves — so all N read
      // the same `next` and went on the wire under ONE seq. The phone's dedupe
      // (correctly) dropped chunks 1..N-1 as duplicates and the transfer died.
      // Under mode ON the same collision is GCM nonce reuse, which is the very
      // thing this module exists to make impossible.
      //
      // The reservation is now a single synchronous read-and-advance, so no
      // two callers can ever be handed the same number regardless of how they
      // interleave. Persist-before-emit is unchanged: the seq is still not
      // RETURNED until its commit has resolved.
      const seq = next;
      next = seq + 1;
      // `sk` is carried from bindKid rather than re-read per frame: a read on
      // the hot path is one more thing that can fail between deciding to send
      // and being allowed to, and the fingerprint cannot change under a sender
      // that is already bound to this kid.
      const mine = tail.then(() => store.commit(seqId(kid, direction), {
        v: SEQ_RECORD_VERSION, kid, direction, next: seq + 1, sk,
      }));
      // The chain must survive a rejection or every later frame would inherit
      // this one's failure as its own; the REAL failure is reported below, to
      // the caller whose commit actually failed.
      tail = mine.catch(() => {});
      try {
        await mine;
      } catch (e) {
        ...
        poisoned = poisoned
          ?? new SeqFailClosedError('commit-failed', `the seq counter could not be persisted: ${e.message}`);
        throw poisoned;
      }
      return seq;
    },
