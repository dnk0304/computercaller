...
/**
 * Key extractor for ACCEPT_PAIRING / PAIRING_ACTIVE / PAIR_STATE:
 * `epk` plus every entry of `recipKeys`.
 *
 * `recipKeys` is the FULL static key set of the pairing — phone, web, and the
 * extension's service worker — not just this recipient's. That is B9: the SAS
 * covers the whole set, so a swapped SW key changes the digits on the side that
 * did not see the swap. A party handed only its own key could not compute the
 * code it is being asked to compare.
 *
 * `wraps` and `kid` are deliberately NOT shape-pinned as keys. A wrap is opaque
 * ciphertext, not a point, so pinning it to 65 bytes would be pinning the wrong
 * thing and would break the moment the AEAD's overhead changed. They are bounded
 * instead — the 4 KB cap already covers the block, and these bounds keep one
 * malformed field from consuming all of it.
 *
 * Throws on a malformed envelope so the caller reports 'badshape'.
 */
function e2eAcceptKeys(block) {
  if (typeof block.kid !== 'string' || block.kid.length === 0 || block.kid.length > 128) {
    throw new Error('kid must be a 1..128 char string');
  }
  const recipKeys = block.recipKeys;
  if (!Array.isArray(recipKeys) || recipKeys.length === 0 || recipKeys.length > 8) {
    throw new Error('recipKeys must be a non-empty array of at most 8 entries');
  }
  const wraps = block.wraps;
  if (!Array.isArray(wraps) || wraps.length === 0 || wraps.length > 8) {
    throw new Error('wraps must be a non-empty array of at most 8 entries');
  }
  for (const w of wraps) {
    if (!w || typeof w !== 'object') throw new Error('wrap must be an object');
    if (typeof w.deviceId !== 'string' || w.deviceId.length === 0 || w.deviceId.length > 128) {
      throw new Error('wrap.deviceId must be a 1..128 char string');
    }
    if (typeof w.wrap !== 'string' || w.wrap.length === 0 || w.wrap.length > 1024) {
      throw new Error('wrap.wrap must be a 1..1024 char string');
    }
  }
  // Every recipient must be wrapped exactly once. A duplicate deviceId would
  // make "the extension's own wrap" ambiguous at the PAIR_STATE fan-out (P1(c)),
  // and the relay is the only party that can see the whole list.
  const ids = wraps.map((w) => w.deviceId);
  if (new Set(ids).size !== ids.length) throw new Error('wraps contains a duplicate deviceId');
  return [block.epk, ...recipKeys];
}
