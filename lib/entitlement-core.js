...
// FREE_TIER flag (2026-09-13, dispatch forge/cardfirst-revert). Read at CALL
// time so a Coolify env edit rolls the gate back on the next request with no
// rebuild. Governs ONLY rule (3) below — see lib/freeTierMode-core.js.
// eslint-disable-next-line @typescript-eslint/no-require-imports -- CJS module (see header).
const { isFreeTierEnabled } = require('./freeTierMode-core.js');
...
function resolveTier(state, subscription) {
  ...
  // FREE TIER (dispatch forge/free-tier-p1, 2026-08-28). A logged-in user with
  // NO subscription row resolves to the `free` tier (state 'free_tier' from
  // rule (3) below). This is the no-card entry point: allowed:true, full app,
  // daily outbound caps. It can NEVER be a grandfathered row (grandfathered
  // requires a subscription), so this branch is only ever reached for genuine
  // new signups — never a paying/pre-launch customer.
  if (state === 'free_tier') return 'free';

  // needs_subscription (FREE_TIER off, not grandfathered) has NO subscription
  // and therefore no planId, so it falls through to planIdToTier(null) =
  // 'solo' — the documented fail-closed least-privilege default. The result
  // is allowed:false regardless; the tier only exists so a caller reading
  // `limits` off a denied result gets the cheapest set, never an unlimited one.
 ...
 * time (a Coolify edit takes effect on the next request, no redeploy).
 * Case-insensitive, whitespace-trimmed.
 *
 * NO HARDCODED FALLBACK (2026-09-17, dispatch forge/w-strip-email-literals).
 * The previous fallback embedded two real email addresses as string literals.
 * Because this module is plain CommonJS it cannot be tree-shaken, so any
 * accidental client-side import shipped those addresses to every visitor in a
 * public `_next/static` chunk (confirmed live). The addresses now live ONLY in
 * the ENTITLEMENT_ALLOWLIST env var.
 *
 * FAIL-CLOSED: unset/empty env ⇒ empty list ⇒ nobody is allowlisted. This is a
 * deliberate reversal of the old NO-LOCKOUT fallback — an allowlist that
 * silently admits people is worse than one that admits nobody, and Dennis
 * retains access via rule (1) isAdminUser (ADMIN_EMAIL) regardless of this list.
 ...
 *   (3) no subscription               → depends on the FREE_TIER flag:
 *         flag ON (default)           → free_tier, ALLOWED (no-card entry point)
 *         flag OFF + grandfathered    → free_tier, ALLOWED (reason
 *                                       'free_tier_grandfathered')
 *         flag OFF + not grandfathered→ needs_subscription, DENIED (card-first)
 ...
 *
 * @param {{ isAdmin: boolean, email: string, freeAccess?: boolean, freeTierGrandfathered?: boolean, subscription: ({ status: string, trialEndsAt: Date, currentPeriodEnd: (Date|null), planId?: (string|null) }|null) }} input
 * @param {Date} [now]
...
function evaluateEntitlementBase(input, now) {
  ...
  const { isAdmin, email, subscription, freeAccess, freeTierGrandfathered } = input;
  ...
  // (3) No subscription at all — FREE TIER (dispatch forge/free-tier-p1,
  // 2026-08-28). PREVIOUSLY this returned {allowed:false, state:'none'} and a
  // logged-in-but-unsubscribed user was fully locked out at /subscribe. Free
  // tier makes "no subscription" the no-card ENTRY POINT instead: allowed:true,
  // state 'free_tier', tier 'free' (via resolveTier) → the user lands in the
  // FULL app with daily outbound caps (20 calls / 10 messages, metered by the
  // relay) and a 14d sync window. This ONLY ever fires for a user whose row was
  // successfully loaded but who has no subscription — i.e. a genuine new signup.
  // It is strictly BELOW admin(1)/allowlist(2)/free_access(2b), so no privileged
  // admit changes; and every PAID/trial state (4/5/6/7) requires a NON-null
  // subscription, so no paying or mid-trial customer can ever reach this branch.
  // NOTE: the "user row not found" path in evaluateUserEntitlement still returns
  // allowed:false state:'none' (fail-closed) — a MISSING account is not free.
  //
  // CARD-FIRST REVERT (2026-09-13, dispatch forge/cardfirst-revert). The branch
  // is now gated by the FREE_TIER flag, with a GRANDFATHER carve-out:
  //
  //   FREE_TIER on (default, unchanged)     → free_tier, allowed (as above).
  //   FREE_TIER off + freeTierGrandfathered → free_tier, allowed. The ~13 users
  //       who signed up under the advertised free offer keep it forever. The
  //       flag is a User column backfilled ONCE, before the flip (see
  //       scripts/backfill-free-tier-grandfather.js) — it is NOT a date compare,
  //       so it cannot drift with the clock and is auditable row by row.
  //   FREE_TIER off + NOT grandfathered     → needs_subscription, DENIED. This
  //       is the card-first entry point: proxy.ts already redirects every
  //       !allowed /app request to /subscribe, which launches Whop checkout →
  //       card → 7-day trial. No new redirect was needed.
  //
  // Still strictly BELOW admin(1)/allowlist(2)/free_access(2b), and every
  // paid/trial state (4-7) requires a NON-null subscription — so no payer,
  // trialist, comped row, or Play-reviewer account can reach this branch in
  // either flag state. `trialDaysLeft` stays null and `allowed` is the only
  // field whose value the flag can change.
  if (!subscription) {
    if (isFreeTierEnabled()) {
      return { allowed: true, state: 'free_tier', trialDaysLeft: null, reason: 'free_tier' };
    }
    if (freeTierGrandfathered === true) {
      return {
        allowed: true,
        state: 'free_tier',
        trialDaysLeft: null,
        reason: 'free_tier_grandfathered',
      };
    }
    return {
      allowed: false,
...
// One-shot startup visibility: a deploy that forgot ADMIN_EMAIL loses the
// email-based admin authority entirely. That must be LOUD in `docker logs`,
// not a silent 403 the next time Dennis opens the admin panel. Logged once per
// process, on the first admin check, so it lands in the container's log without
// spamming every request.
let adminEnvWarned = false;

/**
 * The SINGLE admin authority. True iff the account is flagged isAdmin===true OR
 * its email equals the ADMIN_EMAIL env var (case-insensitive, trimmed).
 *
 * NO HARDCODED FALLBACK (2026-09-17, dispatch forge/w-strip-email-literals) —
 * see isEntitlementAllowed above for why the literal had to go. The admin
 * address now lives ONLY in the ADMIN_EMAIL env var.
 *
 * FAIL-CLOSED throughout: ADMIN_EMAIL unset/empty ⇒ the email route to admin is
 * disabled and ONLY the isAdmin DB flag grants admin; any error / malformed
 * input → false. This is the privileged gate used by the admin page and EVERY
 * /api/admin/* route — deny by default.
 *
 * DEPLOY NOTE: ADMIN_EMAIL must be set in Coolify. Until it is, admin depends
 * solely on the isAdmin DB column.
 *
...
function isAdminUser(account) {
  try {
    ...
    if (!adminEmail) {
      if (!adminEnvWarned) {
        adminEnvWarned = true;
        console.warn('[entitlement] ADMIN_EMAIL unset — admin disabled');
      }
      return false;
    }
    return (
      typeof account.email === 'string' &&
      account.email.trim().toLowerCase() === adminEmail
    );
  } catch {
    return false;
  }
...
/**
 * Coerce a Date | string | number | null into epoch-ms, or null. Used so the
 * grant-active predicates accept both a live Prisma `Date` and an ISO string /
 * epoch (tests, serialized rows) without a second spelling of the compare.
 */
function toMs(v) {
...
/**
 * Is a free-access grant with this `expiresAt` still active at `now`?
 *
 * THE single expiry predicate. `expiresAt == null` = permanent (always active);
 * otherwise active strictly while expiresAt > now (it lapses AT the instant).
 * Shared by isFreeAccessEmail (the relay/browser/relay-ticket gate) AND the
 * admin-feed batch badge (customers/route.ts) so the two can never disagree —
 * the exact drift class of the 2026-08-12 proxy-gate free-access bug.
 *
 * @param {Date|string|number|null|undefined} expiresAt
 * @param {Date|number} [now]
 * @returns {boolean}
 */
function isGrantActive(expiresAt, now) {
  const exp = toMs(expiresAt);
  if (exp == null) return true; // permanent
  return exp > nowToMs(now);
}

/**
 * Derive the display status of a grant: 'permanent' | 'active' | 'expired'.
 * Same source of truth as isGrantActive — the admin API returns this so the UI
 * renders status chips without re-deriving (and drifting from) the rule.
 *
 * @param {Date|string|number|null|undefined} expiresAt
 * @param {Date|number} [now]
 * @returns {'permanent'|'active'|'expired'}
 */
function grantStatus(expiresAt, now) {
  if (toMs(expiresAt) == null) return 'permanent';
  return isGrantActive(expiresAt, now) ? 'active' : 'expired';
}

/**
 * Is this email an ACTIVE free-access grant? The ONE query every gate (relay,
 * browser, admin feed) uses so free-access resolution can never drift — mirrors
 * the tiers/entitlement one-source discipline.
 *
 * Honours the grant's `expiresAt`: a grant whose expiresAt has passed resolves
 * to false (free access auto-lapses at request time — no cron). `now` is
 * threaded from the evaluator so a test can pin it; absent, it defaults to the
 * live clock (matching evaluateEntitlement's own default).
 *
 ...
 */
async function isFreeAccessEmail(dbClient, email, now) {
  ...
  try {
    ...
    if (!row) return false;
    return isGrantActive(row.expiresAt, now);
  } catch {
    return false;
  }
}
...
async function evaluateUserEntitlement(dbClient, userId, now) {
  ...
  try {
    user = await dbClient.user.findUnique({
      ...
      select: {
        isAdmin: true,
        email: true,
        // freeTierGrandfathered (2026-09-13, dispatch forge/cardfirst-revert)
        // — REQUIRED on this path. It is the ONLY thing that keeps a
        // pre-flip free-tier user admitted once FREE_TIER=off. Omitting it
        // silently locks those ~13 accounts out of /app and the relay.
  ...
  const freeAccess = await isFreeAccessEmail(dbClient, user.email, now);
  return evaluateEntitlement(
    {
      isAdmin: user.isAdmin,
      email: user.email,
      freeAccess,
      freeTierGrandfathered: user.freeTierGrandfathered === true,
