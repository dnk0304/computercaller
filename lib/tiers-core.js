 ...
 */
const TIER_LIMITS = {
  // FREE TIER — GRANDFATHERED-ONLY (SPEC-TRIAL-RULES-2026-09-21 §1).
  // Since card-first (2026-09-13) `FREE_TIER=off` in prod: there is no card-free
  // entry point any more. The ONLY users who resolve to this tier are the 13
  // accounts backfilled `freeTierGrandfathered=true` ("let the ones which
  // already are" — Dennis 2026-09-13). state==='free_tier' /
  // 'free_tier_grandfathered' in entitlement-core still resolve here REGARDLESS
  // of planId, and allowed:true — a free user is in the FULL app, not the lock
  // screen.
  //
  // UNMETERED. The two daily OUTBOUND cap keys this set used to carry (20
  // calls and 10 messages per UTC day, forge/free-tier-p1 2026-08-28) were
  // REMOVED 2026-09-21 by trial-caps-purge on Dennis's rule of record: "No call
  // / sms limits for incoming/outgoing calls." No tier in this file or in
  // GRANDFATHERED_TIER_LIMITS carries a per-day cap, the relay no longer meters
  // MAKE_CALL / SEND_SMS, and the refusal frame that went with them is gone.
  // This set is therefore the same SHAPE as every paid set: exactly the four
  // keys below.
  //
  // tests/no-daily-caps.test.js pins all of that as an ABSENCE, by plain
  // substring over this file among others — which is why the removed key names
  // and frame name are described here rather than spelled out. A pin that has
  // to strip comments before it can be trusted is a pin with a second thing
  // that can quietly break.
  //
  // 0 templates / 0 quick-replies fall out of the existing per-tier cap routes
  // (effectiveTemplateLimit returns limits.templates); the 14d sync window is
  // picked up by the existing since-clamp via SYNC_RANGE_WINDOW_DAYS.
  free: {
    templates: 0,
    quickReplies: 0,
    syncRangeMax: '14d',
...
function upgradePathForTier(tier) {
  // free → subscribe to the $5 plan. The upgrade PATH stays (a grandfathered
  // free account can still choose to subscribe); only the reason string changed
  // 2026-09-21 (trial-caps-purge): 'free-limit-hit' named a daily cap that no
  // longer exists. 'free-tier' names the tier the user is ON, which is the
  // actual reason there is somewhere up. Same target as an ending trial, with
  // its own reason so the UI can tell it from a trial upsell. The string is a
  // pass-through in pricing-core.selectUpgradeOffer — no UI branches on it.
  if (tier === 'free') return { reason: 'free-tier', cta: 'subscribe', targetTier: 'plus' };
...
const UPGRADE_REASON = {
  FREE_TIER: 'free-tier', // → subscribe to the $5 plan (user is on the grandfathered free tier)
