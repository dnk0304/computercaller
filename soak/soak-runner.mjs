 ...
 *
 * NOTHING HERE STARTS A CLOCK ON ITS OWN. The window begins when Ken starts the
 * container (R-AM), and `verify-soak.mjs` is what decides whether the window
 * that resulted is a valid 24 h run.
 */
...
const RELAY_WS = process.env.SOAK_RELAY_WS || 'ws://127.0.0.1:3000';
...
// frame exists. The first rehearsal of this runner logged 178 closes and zero
// frames for exactly that reason, so the credentials are not optional garnish:
// without them the soak measures the rejection path for 24 h and reports
// healthy-looking sockets the whole time. See scripts/lib/relay-auth.mjs.
const JWT_SECRET = process.env.JWT_SECRET && process.env.JWT_SECRET.length >= 32
  ? process.env.JWT_SECRET
  : mintSecret();
const EVIDENCE = process.env.SOAK_EVIDENCE_DIR || '/evidence';
const SHA = process.env.SOAK_SHA || 'unset';
const HOURS = Number(process.env.SOAK_HOURS || 24);

// Rule 8 fixes the real cadence at 5 min, and the real run must use it. The
// overrides exist so the rig itself can be REHEARSED in minutes rather than
// only ever exercised for the first time on the one 24 h window that counts —
// an untested soak harness is how a day is spent producing an invalid file.
// verify-soak.mjs grades against the real thresholds regardless, so a rehearsal
// can never be mistaken for a run.
const HEARTBEAT_MS = Number(process.env.SOAK_HEARTBEAT_MS || 5 * 60_000);
const TRACE_MS = Number(process.env.SOAK_TRACE_MS || 60 * 60_000);
const TRAFFIC_MS = Number(process.env.SOAK_TRAFFIC_MS || 30_000);
...
const counters = {
  ...
  opensOff: 0,
  /**
   * Every close, with its code. The deliverable says "zero unexpected closes",
   * so the runner has to be able to tell an expected close from an unexpected
   * one rather than counting them all and hoping. 1000 is a clean close and
   * 4010 is the relay's documented non-terminal code; anything else is
   * unexpected and is recorded individually, with its code and reason, so the
   * trace can be read after the fact instead of just showing a total.
   */
  closesExpected: 0,
  closesUnexpected: 0,
  unexpectedCloseLog: [],
...
const EXPECTED_CLOSE_CODES = new Set([1000, 1001, 4010]);
...
function makePair({ name, mode, urls }) {
  ...
  const wire = (role) => {
    // A FRESH ticket per connect. A cached one survives the first reconnect and
    // then starts failing, which reproduces the 4401 storm this runner exists
    // to avoid — and a soak is all reconnects.
 ...
 * proving cryptography here (that is (a), (e) and (g)'s job) — it is proving
 * the relay carries that shape for a day without growing. Keeping the crypto
 * out of the soak also keeps the soak's own memory profile flat, so a leak the
 * trace shows belongs to the relay and not to the harness watching it.
 */
function tick(pair) {
  const s = pair.seq++;
...
// of them — a 24 h run of two pairs fighting each other, which would read as a
// relay instability that is entirely the harness's own doing.
...
const hb = setInterval(() => {
  // The heartbeat carries enough to prove the pairs were HELD, not merely that
  // a timer fired — a heartbeat that only says "alive" cannot distinguish a
  // running soak from a running clock.
