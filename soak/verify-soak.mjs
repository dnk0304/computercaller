/**
 * The verdict on a soak run. (E2E-P6 (f).)
 *
...
const REQUIRED_HOURS = Number(flag('hours', 24));

const MAX_GAP_MS = 10 * 60_000;   // rule 8
const EXPECT_EVERY_MS = 5 * 60_000;
...
if (files.length > 2) {
  ...
  console.error('refusing more than one heartbeat file: two windows are not one soak (NEW-MA-3)');
  process.exit(2);
}
...
console.log(`       worst gap ${(worstGap / 60000).toFixed(1)} min (expected cadence ${EXPECT_EVERY_MS / 60000} min)`);
...
check(`window spans at least ${REQUIRED_HOURS} h`, spanH >= REQUIRED_HOURS,
...
if (TRACE && fs.existsSync(TRACE)) {
  ...
  if (hourly.length >= 3) {
    ...
    console.log(`       rss ${Math.min(...rss).toFixed(1)}-${Math.max(...rss).toFixed(1)} MB over ${rss.length} samples`);
  }
  ...
  console.log('  note trace file not supplied — memory/CPU trend not checked');
}

console.log(`\n  ${pass} passed, ${fail} failed`);
console.log(fail === 0
  ? `  VERDICT: VALID ${REQUIRED_HOURS} h soak window`
